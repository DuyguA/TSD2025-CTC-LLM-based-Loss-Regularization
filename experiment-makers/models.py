...
class IntermediateWav2Vec2CTC(nn.Module):
    def __init__(self, wav2vec_model, llm_dim):
        super(IntermediateWav2VecCTC, self).__init__()
        
        self.model = wav2vec_model
        self.model.config.output_hidden_states = True
        encoder_dim = self.model.hidden_size
        self.projection_layers = nn.ModuleList([nn.Linear(encoder_dim, llm_dim)
                                           for _ in range(4)]) # 4 intermediate heads
    ...
    def forward(self, input_ids, attention_mask):
        ...
        inters = [6, 12, 18, 24]
        intermediate_outputs  = [self.projection_layers[i](hidden_states[inters[i]]) for i in range(4)]
